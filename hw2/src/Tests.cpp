...
const std::string Tests::testNames[numOfTests] = {
    "testKNNEuclidean", "testKNNManhattan", "testPerceptronLogic",
...
double Tests::testClassifier(std::vector<Point> &subset,
...
void Tests::KNNTestEuclidean(const std::vector<Point> &all) {
  ...
  cout << "KNN Euclidean distance accuracy is:" << evaluateKNN(all, ptrDistance)
       << endl;
  delete ptrDistance;
}
...
void Tests::KNNTestManhattan(const std::vector<Point> &all) {
  ...
  cout << "KNN Manhattan distance accuracy is:" << evaluateKNN(all, ptrDistance)
       << endl;
  delete ptrDistance;
}

void Tests::perceptronTest(const std::vector<Point> &all, double alpha,
                           size_t epochs) {
...
void Tests::perceptronLogic(const std::vector<Point> &all, double alpha,
                            size_t epochs) {
  size_t dimension = all[0].getDimension();
  Perceptron perceptron(dimension, alpha, epochs);
  perceptron.init();
...
void Tests::fixedSplitThree(const std::vector<Point> &all,
                            ...
                            std::vector<Point> &testing) {
  size_t train_size =
      (all.size() * 60) / 100; // 60 percent of samples go to training
  size_t val_size = (all.size() * 10) / 100; // 10 percent to validation
  size_t test_size =
      all.size() - (train_size + val_size); // the rest goes for testing
  std::copy(all.begin(), all.begin() + train_size, back_inserter(training));
  std::copy(all.begin() + train_size, all.begin() + train_size + val_size,
            back_inserter(validation));
...
void Tests::fixedSplitTwo(const std::vector<Point> &all,
                          ...
                          std::vector<Point> &testing) {
  size_t train_size =
      (all.size() * 70) / 100; // 70 percent of samples go to training
  std::copy(all.begin(), all.begin() + train_size, back_inserter(training));
